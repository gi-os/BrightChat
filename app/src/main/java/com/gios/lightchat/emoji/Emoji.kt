 ...
 * find it too. Built by BrightKeyboard's `tools/gen_emoji.py`; see that file for the binary format and for why skin
 * tones, hair and gender are folded onto a base rather than listed separately.
 *
 * Held as flat arrays rather than a list of objects, because the panel
 ...
 *
 * Pure logic, no Android types — `EmojiTableTest` exercises it on the real asset. The one thing this
 * cannot know is whether the *phone* can draw a given glyph, which is a font question; the view
 * filters on that (see `ui/EmojiSheet`). Copied from BrightKeyboard (MIT), asset and all.
 */
